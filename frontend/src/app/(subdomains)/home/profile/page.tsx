...
export default function ProfilePage() {
  ...
  return (
    <main className="relative min-h-screen overflow-x-hidden pt-24 pb-16 px-4 flex items-center justify-center bg-background">
      ...
      <div className="z-10 w-full max-w-lg relative border border-secondary/20 bg-card/30 backdrop-blur-md p-8 rounded-2xl flex flex-col gap-6 shadow-[0_0_30px_rgba(0,150,255,0.08)]">
        ...
        <div className="flex flex-col items-center text-center gap-2 mb-2">
          ...
          <h1 className="text-2xl font-bold font-mono tracking-widest text-foreground">
            OPERATIVE<span className="text-secondary">_CLEARANCE</span>
          </h1>
        ...
        {loading ? (
          <div className="flex flex-col items-center justify-center py-12 gap-3 font-mono text-xs text-muted-foreground">
          ...
          <div className="flex flex-col gap-5 font-mono text-xs">
            {/* Session Info Grid */}
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-4 bg-background/70 border border-border p-4 rounded-xl">
              <div>
                <span className="text-muted-foreground block uppercase text-[9px] font-bold">OPERATIVE_ID</span>
                <span className="text-foreground block truncate">{user.id}</span>
              </div>
              <div>
                <span className="text-muted-foreground block uppercase text-[9px] font-bold">EMAIL_ADDRESS</span>
                <span className="text-foreground block truncate">{user.email}</span>
              </div>
              <div>
                <span className="text-muted-foreground block uppercase text-[9px] font-bold">CLEARANCE_ROLE</span>
                <span className="text-secondary block font-bold">{user.role.toUpperCase()}</span>
              </div>
              <div>
                <span className="text-muted-foreground block uppercase text-[9px] font-bold">ENROLLED_DATE</span>
                <span className="text-foreground block">{user.createdAt}</span>
              </div>
            </div>
            ...
            <div>
              <button
                ...
                className="w-full flex justify-between items-center py-2 px-3 border border-secondary/20 bg-secondary/5 text-[10px] font-bold text-secondary rounded-lg transition-all active:scale-99 hover:bg-secondary/10"
              >
              ...
              {showTokenClaims && rawSession && (
                <pre className="mt-2 bg-background/90 border border-border p-4 rounded-lg text-[9px] text-[#00f0ff] overflow-x-auto whitespace-pre max-h-[160px]">
          ...
          <div className="flex flex-col items-center justify-center text-center py-8 gap-4 font-mono text-xs">
            ...
            <a
              href="/login"
              className="px-6 py-2 border border-secondary text-secondary bg-secondary/5 hover:bg-secondary/15 rounded-lg transition-colors font-bold tracking-wider"
            >
              SIGN_IN_CREDENTIALS
            </a>
          </div>
        ...
        {logs.length > 0 && (
          <div className="bg-background/90 rounded-lg border border-border p-3.5 font-mono text-[9px] text-muted-foreground min-h-[75px] max-h-[110px] overflow-y-auto space-y-1">
            {logs.map((log, index) => (
